...
async function getNextTripNumber(plateId) {
  const row = await queryOne('SELECT MAX(trip_number) as max FROM trips WHERE plate_id = ?', [plateId]);
  return (row?.max || 0) + 1;
}

async function getNextInvoiceNumber() {
  const prefix = (await queryOne("SELECT value FROM settings WHERE key = 'invoice_prefix'"))?.value || 'DWF-';
  const row = await queryOne('SELECT COUNT(*) as cnt FROM trips');
...
router.get('/:id', auth, async (req, res) => {
  const t = await queryOne(`SELECT t.*, c.name as client_name, p.plate as plate_number
...
router.post('/', auth, requireRole('super_admin', 'data_entry'), async (req, res) => {
  ...
  const client = await queryOne('SELECT * FROM clients WHERE id = ? AND is_active = 1', [client_id]);
  if (!client) throw new AppError(400, 'Client not found or inactive');
  const plate = await queryOne('SELECT * FROM plate_numbers WHERE id = ? AND is_active = 1', [plate_id]);
  if (!plate) throw new AppError(400, 'Plate not found or inactive');

  const existing = await queryOne('SELECT id FROM trips WHERE trip_date = ? AND trip_time = ? AND plate_id = ?', [trip_date, trip_time, plate_id]);
  if (existing) throw new AppError(409, 'Duplicate trip: same date, time, and plate already exists');

  let result;
  await transaction(async () => {
    const tripNumber = await getNextTripNumber(plate_id);
    ...
    const deposit = await queryOne('SELECT SUM(remaining) as total FROM deposits WHERE client_id = ?', [client_id]);
    ...
    const insertResult = await execute(`INSERT INTO trips (client_id, plate_id, trip_number, invoice_number, trip_date, trip_time, item, amount, payment_status, note, deposit_used)
      VALUES (?,?,?,?,?,?,?,?,?,?,?) RETURNING id`,
    ...
    if (depositUsed > 0) {
      ...
      const depositRows = await queryAll('SELECT id, remaining FROM deposits WHERE client_id = ? AND remaining > 0 ORDER BY created_at ASC', [client_id]);
      for (const d of depositRows) {
        if (remaining <= 0) break;
        const deduct = Math.min(d.remaining, remaining);
        await execute('UPDATE deposits SET remaining = remaining - ? WHERE id = ?', [deduct, d.id]);
        remaining -= deduct;
      }
    }

    const trip = await queryOne(`SELECT t.*, c.name as client_name, p.plate as plate_number
      FROM trips t JOIN clients c ON t.client_id = c.id JOIN plate_numbers p ON t.plate_id = p.id WHERE t.id = ?`, [insertResult.rows[0].id]);

    await updateClientBalance(client_id);
    result = trip;
  });
...
router.put('/:id', auth, requireRole('super_admin'), async (req, res) => {
  const t = await queryOne('SELECT * FROM trips WHERE id = ?', [req.params.id]);
  if (!t) throw new AppError(404, 'Trip not found');
  const { amount, payment_status, note } = req.body;
  await transaction(async () => {
    await execute('UPDATE trips SET amount=?, payment_status=?, note=? WHERE id=?',
      [amount !== undefined ? amount : t.amount, payment_status || t.payment_status, note !== undefined ? note : t.note, req.params.id]);
    await updateClientBalance(t.client_id);
  });
  res.json(await queryOne(`SELECT t.*, c.name as client_name, p.plate as plate_number
    FROM trips t JOIN clients c ON t.client_id = c.id JOIN plate_numbers p ON t.plate_id = p.id WHERE t.id = ?`, [req.params.id]));
});
...
async function updateClientBalance(clientId) {
  const unpaid = await queryOne('SELECT COALESCE(SUM(amount - deposit_used), 0) as total FROM trips WHERE client_id = ? AND (payment_status = \'Unpaid\' OR payment_status = \'Outstanding\')', [clientId]);
  const paid = await queryOne('SELECT COALESCE(SUM(amount), 0) as total FROM trips WHERE client_id = ? AND payment_status = \'Paid\'', [clientId]);
  const payments = await queryOne('SELECT COALESCE(SUM(amount), 0) as total FROM payments WHERE client_id = ?', [clientId]);
  const depositsTotal = await queryOne('SELECT COALESCE(SUM(amount), 0) as total FROM deposits WHERE client_id = ?', [clientId]);
  const depositsRemaining = await queryOne('SELECT COALESCE(SUM(remaining), 0) as total FROM deposits WHERE client_id = ?', [clientId]);
  ...
  await execute('UPDATE clients SET balance = ? WHERE id = ?', [balance, clientId]);
}
