...
router.post('/', auth, requireRole('super_admin', 'data_entry'), async (req, res) => {
  ...
  await transaction(async () => {
    const result = await execute('INSERT INTO payments (client_id, amount, payment_date, note) VALUES (?,?,?,?) RETURNING id',
    ...
    payment = await queryOne('SELECT p.*, c.name as client_name FROM payments p JOIN clients c ON p.client_id = c.id WHERE p.id = ?', [result.rows[0].id]);
  });
...
router.delete('/:id', auth, requireRole('super_admin'), async (req, res) => {
  const p = await queryOne('SELECT * FROM payments WHERE id = ?', [req.params.id]);
  if (!p) throw new AppError(404, 'Payment not found');
  await transaction(async () => {
    await execute('DELETE FROM payments WHERE id = ?', [req.params.id]);
    const unpaid = await queryOne('SELECT COALESCE(SUM(amount - deposit_used), 0) as total FROM trips WHERE client_id = ? AND (payment_status = \'Unpaid\' OR payment_status = \'Outstanding\')', [p.client_id]);
    const paid = await queryOne('SELECT COALESCE(SUM(amount), 0) as total FROM trips WHERE client_id = ? AND payment_status = \'Paid\'', [p.client_id]);
    const payments = await queryOne('SELECT COALESCE(SUM(amount), 0) as total FROM payments WHERE client_id = ?', [p.client_id]);
    const depositsRemaining = await queryOne('SELECT COALESCE(SUM(remaining), 0) as total FROM deposits WHERE client_id = ?', [p.client_id]);
    const totalCharged = (unpaid?.total || 0) + (paid?.total || 0);
    const totalPaid = (payments?.total || 0) + (paid?.total || 0);
    const balance = totalCharged - totalPaid - (depositsRemaining?.total || 0);
    await execute('UPDATE clients SET balance = ? WHERE id = ?', [balance, p.client_id]);
  });
